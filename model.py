...
def sim_neurons(Ie_mx,W,T,dt,stdp=True,inhs=None): #current array,weight matrix, max time, time step
    ...
    def alpha_m(V):return  0.1*(V+40)/(1-np.exp(-(V+40.001)/10)) 
    def beta_m (V):return 4*np.exp(-(V+65.001)*(139/2500))
    def alpha_h(V):return  0.07*np.exp(-0.05*(V+65.001))
    def beta_h (V):return 1/(1+np.exp(-0.1*(V+35.001)))
    def alpha_n(V):return  0.01*(V+55.001)/(1-np.exp(-0.1*(V+55.001)))
    def beta_n (V):return 0.125*np.exp(-0.0125*(V+65.001))
