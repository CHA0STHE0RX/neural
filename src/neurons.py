...
#IUAON/CHA0STHE0RX
def sim_neurons(Ie_mx,W,T,dt): #current array,weight matrix, max time, time step
    ...
    s= np.full(N_neurons,0.0)
    t=np.arange(0,T,dt)
    t_steps=len(t)
    V = np.zeros((t_steps,N_neurons))
    V[0,:]=-65   # Initial voltage
    m= np.full(N_neurons,0.05)
    h= np.full(N_neurons,0.6)
    n= np.full(N_neurons,0.32)
    ...
    for i in range(1,len(t)):
        ...
        dV=(-G_L*(lV-V_L) -G_Na*m**3*h*(lV-E_Na) - G_K*n**4*(lV-E_K)+Ie-I_syn)*dt/C
